...
$(function() {
	...
	$('.contact-form').on("submit", function(){
        ...
        if ($.trim($('.contact-form input[name="email"]').val()) === '') {error = 1; $('.contact-form input[name="email"]').addClass('error-class'); msg = msg +  '\n - Email';}
        if ($.trim($('.contact-form input[name="company"]').val()) === '') {error = 1; $('.contact-form input[name="company"]').addClass('error-class'); msg = msg +  '\n - Company Name';}
        if (!$.trim($('.contact-form input[name="subject"]').val()) === '') {error = 1; $('.contact-form input[name="subject"]').addClass('error-class'); msg = msg +  '\n - Subject';}
        //if (!pattern.test($.trim($('.contact-form input[name="companyname"]').val()))) {error = 1; $('.contact-form input[name="companyname"]').addClass('error-class'); msg = msg +  '\n - Company Name';}
        //if (!pattern.test($.trim($('.contact-form input[name="subject"]').val()))) {error = 1; $('.contact-form input[name="subject"]').addClass('error-class'); msg = msg +  '\n - Subject';}
		if ($.trim($('.contact-form textarea[name="message"]').val()) === '') {error = 1; $('.contact-form textarea[name="message"]').addClass('error-class'); msg = msg +  '\n - Your Message';}
        ...
        if (error){
            ...
            var url = 'reachUs.aspx',
            	...
            	text = $.trim($('.contact-form textarea[name="message"]').val()),
            	mailto = $.trim($('.contact-form input[name="mailto"]').val());

            $.post(url,{'name':name,'email':email,'subject':subject,'company':company,'text':text,'mailto':mailto},function(data){
